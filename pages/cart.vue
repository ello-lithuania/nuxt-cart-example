        ...
        <v-col md="8">
            <v-card class="py-5 px-4" style="max-height: 500px; overflow-y: auto;">
                        ...
                        <v-row>
                            <v-col sm="4">
                                <div class="d-flex align-center no-wrap">
                                    <img style="width:90px; height: 90px; object-fit: cover;" :src="item.image"/>
                            ...
                            <v-col sm="8">
                                <div class="d-flex justify-space-between align-center no-wrap w-full h-100">
                                    <div class="d-flex align-center bg-primary rounded">
                                        <span class="icon d-flex align-center pa-3" style="cursor:pointer" @click="cartStore.add(item.id)">
                                            <v-icon>mdi-plus</v-icon>
                                        </span>
                                        <span>
                                            {{  item.quantity }}
                                        </span>
                                        <span style="cursor: pointer" class="icon d-flex align-center pa-3" @click="cartStore.remove(item.id)">
                                            <v-icon>mdi-minus</v-icon>
                                        </span>
                                    </div>
                                    <p>{{ item.quantity }} X {{ item.price }} = <strong>{{ item.cost }}</strong></p>
                ...
                <v-card-text class="text-center" v-else>
                    <v-btn class="mx-auto" @click="$router.push('/')" color="primary">Your cart is empty. Fill it</v-btn>
                </v-card-text>
            </v-card>
        </v-col>
